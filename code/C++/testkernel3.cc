...
int main () {
  ...
  std::vector<double> r        = kernel3_compute<uint32_t, csc_matrix<uint32_t>, k3_once_base<uint32_t>>(3, nonzeros, 1, &initial_r);
  std::vector<double> rcilk    = kernel3_compute<uint32_t, csc_matrix<uint32_t>, k3_once_cilk<uint32_t, 1>>(3, nonzeros, 1, &initial_r);
  std::vector<double> rcilkfor = kernel3_compute<uint32_t, csc_matrix<uint32_t>, k3_once_cilkfor<uint32_t>>(3, nonzeros, 1, &initial_r);
  std::vector<double> romp     = kernel3_compute<uint32_t, csc_matrix<uint32_t>, k3_once_omp<uint32_t>>(3, nonzeros, 1, &initial_r);
